...
public class Test_User_Name_Present_In_Profile_Logout implements AutomationConstants {
			...
			try
			{
				...
				try {
				   ...
				   // driver.navigate().to("http://localhost:5173/about-us");
				    ...
				    WebElement profileBtn = driver.findElement(By.xpath("//div[@class='relative']/div/button[2]"));
				    driver.manage().timeouts().implicitlyWait(Duration.ofSeconds(10));
				    profileBtn.click();
				    driver.manage().timeouts().implicitlyWait(Duration.ofSeconds(10));
				    wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//header//a[text()='Login']")));

				    List<WebElement> nameCheck = driver.findElements(By.xpath("//header//span[contains(text(), 'shilpa')]"));
				    Assert.assertTrue(nameCheck.isEmpty());

				    System.out.println("✅ Username disappeared after logout, 'Login' reappeared.");
				    String expectedTitle1 = "Login | ECODERS";
