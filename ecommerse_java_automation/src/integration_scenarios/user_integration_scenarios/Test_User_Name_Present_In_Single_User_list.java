...
public class Test_User_Name_Present_In_Single_User_list implements AutomationConstants {
...
//					System.out.println("name present in excel   :"+NameList);
				 ...
				 // ✅ Collect user names from WebElements into a String list
				    List<WebElement> allusrs = driver.findElements(
				        By.xpath("//a[@data-discover='true']/h3")
				    );

				    List<String> userNames = new ArrayList<>();

				    for (WebElement user : allusrs) {
				        String name = user.getText().trim();
				        System.out.println(name);
				        userNames.add(name);
				        System.out.println("-------------------------------");
				    }
				   System.out.println( userNames.contains("shillu"));
				   Boolean result=userNames.contains("shillu");
